...
class mpasien extends CI_Model{
 ...
 function cek_rm($no_rm,$dokter){
    ...
    function simpan($id, $no_rm, $nama, $alamat, $tgl_lahir, $telp, $telp2, $keluarga_dekat, $kelamin, $operasi ,$dokter ,$status, $tgl_antri, $tgl_permintaan,  $prioritas, $ket_prioritas, $ket_panggilan, $hak_kelas, $kelas_diminta, $jam ,$tgl_realisasi,  $penanggung, $diagnosa, $user ){   
   if ($status == 0 ) {
    $data = array(
        'id'=>$id,
        'no_rm'=>$no_rm,
    ...
    function hapus($id){
     ...
     function code_pasien(){
            $this->db->select('Right(pasien.no_rm,6) as no_rm ',false);
            ...
            $kodemax = str_pad($no_rm,6,"0",STR_PAD_LEFT);
            $kodejadi  = $kodemax;
            return $kodejadi;
